...
export class __WasmPointer() {
   ...
   */
  constructor(addr, convert, size, type) {
    this.addr = addr;
  ...
  deref() {
    ...
    switch (this.type) {
      case 'i8':
        return new Int8Array(__wasm_memory.buffer)[this.addr]
      case 'i16':
        return new Int16Array(__wasm_memory.buffer)[this.addr / 2]
      case 'i32':
        return new Int32Array(__wasm_memory.buffer)[this.addr / 4]
      case 'i64':
        return new BigInt64Array(__wasm_memory.buffer)[this.addr / 8]
      case 'u8':
        return new Uint8Array(__wasm_memory.buffer)[this.addr]
      case 'u16':
        return new Uint16Array(__wasm_memory.buffer)[this.addr / 2]
      case 'u32':
        return new Uint32Array(__wasm_memory.buffer)[this.addr / 4]
      case 'u64':
        return new BigUint64Array(__wasm_memory.buffer)[this.addr / 8]
      case 'f32':
        return new Float32Array(__wasm_memory.buffer)[this.addr / 4]
      case 'f64':
        return new Float64Array(__wasm_memory.buffer)[this.addr / 8]
      default:
        return false
        //return __wasm_memory.buffer.slice(this.addr, this.addr + this.size)
    }
  }
  ...
  assign(val) {
    ...
    switch (this.type) {
      case 'i8':
        new Int8Array(__wasm_memory.buffer)[this.addr] = val
      case 'i16':
        new Int16Array(__wasm_memory.buffer)[this.addr / 2] = val
      case 'i32':
        new Int32Array(__wasm_memory.buffer)[this.addr / 4] = val
      case 'i64':
        new BigInt64Array(__wasm_memory.buffer)[this.addr / 8] = val
      case 'u8':
        new Uint8Array(__wasm_memory.buffer)[this.addr] = val
      case 'u16':
        new Uint16Array(__wasm_memory.buffer)[this.addr / 2] = val
      case 'u32':
        new Uint32Array(__wasm_memory.buffer)[this.addr / 4] = val
      case 'u64':
        new BigUint64Array(__wasm_memory.buffer)[this.addr / 8] = val
      case 'f32':
        new Float32Array(__wasm_memory.buffer)[this.addr / 4] = val
      case 'f64':
        new Float64Array(__wasm_memory.buffer)[this.addr / 8] = val
      default:
        return false
        //return __wasm_memory.buffer.slice(this.addr, this.addr + this.size)
    }
